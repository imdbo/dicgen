...
plurals = [word.pluralize() for word in blob.words]
    ...
    def _n_frequent_words(self, w2v:object, binary: bool = True, number_lemmas:int = 100000):
        '''
    ...
    def generate_def(self, hw: str, l_entry:str, disambiguation:bool = False):
        max_size_f_entry = 0 
        cut_entry = []
        ...
        l_entry = cut_entry 
        if "may refer to" not in l_entry:
            ...
            if disambiguation == False:
                ...
                if hw not in self.disambiguations:
                    self.disambiguations[hw] = l_entry
                else:
                    self.disambiguations[hw].append(l_entry)
                                
            if hw not in self.lemma_inflection:
                self.lemma_inflection[hw] = self.inflect(hw, full_article)

    def build(self):
        full_df = pd.read_csv(self.data_df, low_memory=False, chunksize=10000)
        disambiguation = False
        for df in full_df:
            headwords = np.array(df['headword'])
            full_articles = np.array(df['long_entry'])
            '''
            ...
            for i in tqdm(range(len(headwords))):
                ...
                if isinstance(hw, str) and hw.lower() in self.lemma_index:
                    ...
                    self.generate_def(l_entry = l_entry, hw = hw, disambiguation=disambiguation)
                '''
                error log missing 
                else:
                ''' 
